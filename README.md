...
1. **Preprocess raw EEG**  
   ...
   with aligned labels and RTs. Point downstream scripts (for example `plot_topomap.py`) to `<deriv_root>`.

2. **Technical validation — signal quality**  
   Run `technical_validation/signal_quality/plot_preproc_interp_and_ica_violins.py` for interpolated-channel  
   and rejected-IC summaries (requires `preproc_violin_export/preproc_summary_per_subject.csv` under the  
   same folder, aggregated from preprocessing outputs).  
   Run `technical_validation/signal_quality/plot_subject_ica_components.py --subject-name <name> --file-index 1 --output-root <deriv_root>`  
   for representative rejected ICA components.  
   Set `DATA_ROOT` in `technical_validation/signal_quality/plot_topomap.py` to `<deriv_root>`, then run it  
   for group PSD and band topographies (baseline vs stimulus).

3. **Technical validation — behavior**  
   Configure input CSV paths in each script (or place files under the expected `rt_exports/` folders), then run  
   `technical_validation/behavior/plot_rt_excluded_violin.py` for excluded trials (RT > 5000 ms);  
   `technical_validation/behavior/plot_rt_session.py` for RT distributions across sessions;  
   `technical_validation/behavior/category_attraction.py` for category-level selection rates;  
   `technical_validation/behavior/plot_pair_aggregate_glm_forest.py` for the pair-level acoustic-feature  
   GLM forest plot (requires `pair_aggregate_ols_out/glm_coefficients.csv`).

4. **Technical validation — baseline decoding**  
   Set `--data-root` to `<deriv_root>` for all scripts below.  
   - Random split: `technical_validation/aad_baseline/eegnet_2s_cv5.py`  
   - Mirror-constrained split: `technical_validation/aad_baseline/eegnet_2s_cv5.py --pair-consistent`  
     (or `technical_validation/aad_baseline/Mirror- and category-held-out CV.py` with default mirror-constrained settings)  
   - Category-held-out split: `technical_validation/aad_baseline/Mirror- and category-held-out CV.py --subcategory-6fold`  
   Summarize the three schemes with `technical_validation/aad_baseline/plot_bacc_three_strategies_boxplot.py`.  
   `pair_consistent_splits.py` provides fold definitions for the mirror and category-held-out scripts and is  
   imported by them rather than run standalone.
...
- Trial labels are **post-stimulus self-reports** of the more attention-capturing side.
